...
//	}
	...
	public ProductSummary getProductSummary() {
		...
		List<ReceiptDetails> products = productRepo.findAll();
		double totalPrice = 0;
		double totalTax=0;
		for(ReceiptDetails product : products){
			totalPrice+=product.getProductTotalPrice();
			totalTax+=product.getProductTotalTax();
		}
		totalPrice=(totalPrice*100)/100.00;
		productFinalReceipt.setTotalPrice(totalPrice);
		productFinalReceipt.setTotalTax(totalTax);
		productFinalReceipt.setProducts(products);
